...
1. **世界数据全部自己定义**：Transform、Renderable、Collider、RigidBody authoring/state 都在 `src/world.h` 里，是你自己的数据结构。
...
- 你以后移植到 Java / JS / 其他项目时，最容易迁走的是这一层；
...
你以后替换算法时，外部接口不必变，只需要在 `Step()` 里逐步落下：
...
所以你后面研究半隐式欧拉、冲量求解、PGS、关节系统时，直接把 `AdvancePhysicsOneTick()` 这条链接上即可。
...
- 你以后换 raylib / SDL + OpenGL / WebGL，不会重写物理；
- 你以后换物理实现，不会重写世界数据；
- 你以后移植算法时，最重要的数据定义不会和库 API 缠死。
...
这是你的主战场。当前已有一个最小刚体骨架，下一步是 manifold / 角冲量 / 关节。

### 常改 2：`src/world.h`

这里决定了你未来移植时的数据边界长什么样。
...
你这次不要追求“先写一个完整游戏引擎”，而要追求：

- 一套可信的主循环
- 一套你自己完全掌控的数据定义
- 一个可以暂停、单步、观察状态的 3D 壳子
- 一个可以被你慢慢替换成真实求解器的 physics backend

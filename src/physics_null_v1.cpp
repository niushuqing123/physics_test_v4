...
//      - no joints
//
// This is not an industrial engine, but it is a good box rigid body V1 base.
// ============================================================================
...
constexpr float kVelocityBaumgarte = 0.20f;
constexpr float kPositionBaumgarte = 0.80f;
...
private:
    ...
    void InitializeContact(Contact& c, float dt) {
        ...
        c.restitution = 0.0f;
        c.bias = (dt > 0.0f) ? (kVelocityBaumgarte / dt) * std::max(c.penetration - kPenetrationSlop, 0.0f) : 0.0f;
    ...
    void GenerateContacts(float dt) {
        ...
        for (const BroadphasePair& pair : broadphase_pairs_) {
            ...
            for (const ContactPointCandidate& candidate : candidates) {
                ...
                c.penetration = std::max(candidate.penetration, sat.penetration);
    ...
    void SolvePositionConstraints() {
        for (Contact& c : contacts_) {
            ...
            const std::uint32_t b = c.b;
            const float inv_mass_a = inverse_mass_[a];
            const float inv_mass_b = inverse_mass_[b];
            const float inv_mass_sum = inv_mass_a + inv_mass_b;
            if (inv_mass_sum <= kEpsilon) continue;

            const float fresh_pen = RemeasurePenetration(obb_proxy_[a], obb_proxy_[b], c.normal);
            if (fresh_pen <= 0.0f) {
                continue;
            }

            const float correction_mag = kPositionBaumgarte * std::max(fresh_pen - kPenetrationSlop, 0.0f) / inv_mass_sum;
            const Vec3 correction = c.normal * correction_mag;

            if (inv_mass_a > 0.0f) position_[a] -= correction * inv_mass_a;
            if (inv_mass_b > 0.0f) position_[b] += correction * inv_mass_b;

            UpdateDerivedBodyState(a);
            UpdateDerivedBodyState(b);

            const Vec3 rv = linear_velocity_[b] - linear_velocity_[a];
            const float vn = Dot(rv, c.normal);
            if (vn < 0.0f) {
                const float remove = vn / inv_mass_sum;
                const Vec3 impulse = c.normal * remove;
                if (inv_mass_a > 0.0f) linear_velocity_[a] += impulse * inv_mass_a;
                if (inv_mass_b > 0.0f) linear_velocity_[b] -= impulse * inv_mass_b;
            }
        }
    }

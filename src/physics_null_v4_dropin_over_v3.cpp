...
//      - no joints
//
// This is still intentionally small, but now it also adds a very small amount of
// temporal coherence (normal-only warm starting + lightweight manifold cache) and
// a more stable box-box contact path for resting contacts.
//
// v4 focus:
//   - keep v3.1's normal-only warm start because it fixed the pyramid collapse
//   - prefer face manifolds over fragile edge-edge picks when overlaps are close
//   - use cached pair normals as a light SAT hysteresis hint across frames
//   - switch velocity iterations to friction-first / normal-last
//
// Notes for the later ragdoll phase:
//   - this file still keeps COM-centric body poses, local collider poses, point velocities
//     and world-space inverse inertia, which are the exact primitives future joint constraints
//     will need; v4 does not add joints yet, but it stays on the right data path for them.
// ============================================================================
...
constexpr float kSatEdgeAxisBias = 0.05f;
constexpr float kSatPreferredNormalDotMin = 0.96f;
constexpr float kSatPreferredNormalBias = 0.015f;
...
BoxBoxSatResult BoxBoxSat(const BoxWorldProxy& a, const BoxWorldProxy& b, const Vec3* preferred_normal) {
    ...
    auto test_axis = [&](Vec3 axis, int axis_index, int edge_a, int edge_b) -> bool {
        ...
        float selection_metric = overlap;
        if (axis_index >= 6) {
            selection_metric += kSatEdgeAxisBias;
        }
        if (preferred_normal != nullptr) {
            const float alignment = Dot(axis, *preferred_normal);
            if (alignment > kSatPreferredNormalDotMin) {
                const float t = Clamp((alignment - kSatPreferredNormalDotMin) / (1.0f - kSatPreferredNormalDotMin), 0.0f, 1.0f);
                selection_metric -= kSatPreferredNormalBias * t;
            }
        }
...
public:
    const char* Name() const override {
        return "NullPhysicsBackendV4_DropinOverV3";
    }
...
private:
    ...
    // This 1-DOF effective-mass helper is deliberately generic: contact normals,
    // friction tangents and future ragdoll joint axes can all reuse the same math.
    ...
    void GenerateContacts(float dt) {
        ...
        for (const BroadphasePair& pair : broadphase_pairs_) {
            ...
            Vec3 preferred_normal{0.0f, 0.0f, 0.0f};
            const Vec3* preferred_normal_ptr = nullptr;
            const auto previous_it = previous_contact_cache_.find(pair_key);
            if (previous_it != previous_contact_cache_.end()
             && LengthSqNoNaN(previous_it->second.normal) > kEpsilon * kEpsilon) {
                preferred_normal = previous_it->second.normal;
                preferred_normal_ptr = &preferred_normal;
            }

            const BoxBoxSatResult sat = BoxBoxSat(obb_proxy_[a], obb_proxy_[b], preferred_normal_ptr);
            if (sat.separated) {
                continue;
            }
    ...
    void SolveVelocityConstraints() {
        for (Contact& c : contacts_) {
            ...
            const float jt1 = -Dot(rv, c.tangent1) * c.tangent1_effective_mass;
            const float jt2 = -Dot(rv, c.tangent2) * c.tangent2_effective_mass;
            ...
            const Vec3 va_after_t = ComputePointVelocity(c.a, c.r1);
            const Vec3 vb_after_t = ComputePointVelocity(c.b, c.r2);
            const float normal_speed = Dot(vb_after_t - va_after_t, c.normal);
